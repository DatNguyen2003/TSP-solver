...
public class AI_project3_DatNguyen {
  private static final int numsGen = 50;
  private static final int numsPathGen = 50;
    ...
    try {
      ...
      for (int i = 0; i < numsGen; i++) {
        ...
        while (nextGens.size() < gens.size()) {//Select Paths to crossover and add their children to the next generation
          int parent1Index = rand.nextInt(numsCities/4);
          int parent2Index = rand.nextInt(numsCities);
        ...
        int numsMutate = rand.nextInt(numsCities);//Mutate a random number of Paths with the highest cost in the next generation 
